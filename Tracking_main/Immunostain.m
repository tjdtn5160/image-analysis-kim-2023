...
%cmosoffsetfile=['\Volumes\HW_Data\3. Processing data\CMOS_Offset\IXliu\cmosoffset_bin' num2str(binning) '.mat'];

saveshot=[num2str(row),'_',num2str(col),'_',num2str(site)];
%saveshot=[rowstring,colstring,'_',sitestring];
...
%     debrisarea=300;
%     boulderarea=1200;
% elseif ismember(col,1:2) & ismember(row,1:4)
%     nucr=10; %
%     eccentricitythresh=0.90;
%     debrisarea=300;
%     boulderarea=1200;
...
for cc=1:numgood
    ...
    %     sigB1(cc)=mean(realB1(nuc_info(cc).PixelIdxList));
    %     sigB2(cc)=median(realB2(nuc_info(cc).PixelIdxList));
    %     sigB3(cc)=median(realB3(nuc_info(cc).PixelIdxList));
    %     if ismember(row,2:4)
    %         sigB4(cc)=median(realB4(nuc_info(cc).PixelIdxList));
    %     end
    
    %if puncta
    %   if (cc<=numel(puncta_info_low_A2))
    %      sigA2arealow(cc)=puncta_info_low_A2(cc).Area;
    %     sigA2sumlow(cc)=sum(realA2(puncta_info_low_A2(cc).PixelIdxList));
    %    sigA2countlow(cc)=punctacount_low_A2(cc);
    %end
    %if (cc<=numel(puncta_info_high_A2))
    %    sigA2areahigh(cc)=puncta_info_high_A2(cc).Area;
    %    sigA2sumhigh(cc)=sum(realA2(puncta_info_high_A2(cc).PixelIdxList));
    %    sigA2counthigh(cc)=punctacount_high_A2(cc);
    %end
    %if (cc<=numel(puncta_info_low_A3))
    %   sigA3arealow(cc)=puncta_info_low_A3(cc).Area;
    %  sigA3sumlow(cc)=sum(realA3(puncta_info_low_A3(cc).PixelIdxList));
    % sigA3countlow(cc)=punctacount_low_A3(cc);
    %end
    %if (cc<=numel(puncta_info_high_A3))
    %   sigA3areahigh(cc)=puncta_info_high_A3(cc).Area;
    %  sigA3sumhigh(cc)=sum(realA3(puncta_info_high_A3(cc).PixelIdxList));
    % sigA3counthigh(cc)=punctacount_high_A3(cc);
    %end
    %end
...
%showImagesWithLinkedAxes({realA1,realA2})
%showImagesWithLinkedAxes({realA1,realA2,realA3,realA4})
...
%=sigringcalc_cdk4([],nuc_label_all,innerrad,outerrad,numgood,realA3,ringthresh,MAXthresh);
