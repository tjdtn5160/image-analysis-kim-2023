...
%     prebuffer=5;
    ...
    sig_fwdslope(1:50)=0; % cells very unlikely to enter into s phase within 10hours after serum release, solves the hump issue
    altstore(i,firstframe(i):lastframe(i))=sig_fwdslope;
    tempsearch=find(sig_fwdslope>0.05 & abs(signal)<200,1,'last'); % default threshold 0.05 and 0.15 (if normalized) %200 for mcf10a, 50 for bj5
