...
clear all;
clear; clc;
root='D:\1. Projects\2. Melanoma project\2020-07-29 (Melanoma_OptoFGFR1_MEK1,2,ERK1,2 siRNA_BRAFi,MEKi_Live)\';
datadir=[root 'Data'];
resultdir=[root 'Results'];
if ~exist(resultdir)
    mkdir(resultdir);
end
option=1; % 1: WM858, 2: WM983B
...
%root='/Volumes/1. Projects/2. Melanoma project/2020-06-11 (Melanoma_OptoFGFR1_BRAFi,+ERKi,MEKi_CFP100% 100ms, 1min int, 2.5hr)/';
%resultdir=[root 'Results/'];

load([resultdir 'CombinedData_' num2str(option)]);

%% Select condition of experiments
IFoption=0; noMitosis=1; drugFrame=10;
numCond=length(ret);
label={'Vem','Vem+M1','Vem+M2','Vem+M1,2','Vem+E1,2','Vem+Cobi','Vem+Cobi+M1','Vem+Cobi+M2','Vem+Cobi+M1,2','Vem+Cobi+E1,2','Vem+Trame','Vem+Trame+M1','Vem+Trame+M2','Vem+Trame+M1,2','Vem+Trame+E1,2','Dab','Dab+M1','Dab+M2','Dab+M1,2','Dab+E1,2','Dab+Cobi','Dab+Cobi+M1','Dab+Cobi+M2','Dab+Cobi+M1,2','Dab+Cobi+E1,2','Dab+Trame','Dab+Trame+M1','Dab+Trame+M2','Dab+Trame+M1,2','Dab+Trame+E1,2'};
for condi=1:numCond;
    condition=condi;
    ...
    if IFoption
        ...
        IFdata=ones(size(data,1),1).*NaN;
    end
    D = sort_or_align_by_mitosis(data,divFrame,IFoption,IFdata,noMitosis)
    %% #1, Checking manually, Clear bad traces (Manually)
    % D.clean_manually_remove_data(1,0); % Data Set, Aligned by mitosis
    % D.clean_manually_remove_data(4,1); % 
    ...
    %D.clean_leastnum_frames(50); % this will remove frames shorter than x.
    D.clean_leastnum_frames_5(1,drugFrame,3);
    D.clean_leastnum_frames_5(11,70,30);
    ...
    %% Classify responders and nonresponders
    data=D.dataSorted(:,:,1);
    cellNum=size(data,1);

        tres=zeros(cellNum,1);
        for j=1:cellNum
            tres(j)=ttest2(data(j,1:drugFrame-1),data(j,drugFrame+1:drugFrame+30),.001,'left');
            normdata(j,:)=data(j,:)/nanmean(data(j,1:drugFrame-1));
        end
    gating=logical(tres);
    %%
    sensor(condi).gating=gating;
    sensor(condi).ERK=data;
    sensor(condi).normERK=normdata;
    %sensor(condi).ERK_corrected=ERK_activity_correction_allcells(sensor(condi).ERK,sensor(condi).CDK2);
    %sensor(condi).CDK4_corrected=CDK4_activity_correction_allcells(sensor(condi).CDK4,sensor(condi).CDK2);
    clear gating; clear data; clear normdata;
end
%% Save data
savedir=[root 'Results' filesep];
if ~exist(savedir,'dir')
    mkdir(savedir);
end
save([savedir filesep 'Data_' num2str(option) '.mat'],'sensor','label','drugFrame');
%%
option=2;
root='D:\1. Projects\2. Melanoma project\2020-07-29 (Melanoma_OptoFGFR1_MEK1,2,ERK1,2 siRNA_BRAFi,MEKi_Live)\';
...
%root='/Volumes/1. Projects/2. Melanoma project/2020-06-11 (Melanoma_OptoFGFR1_BRAFi,+ERKi,MEKi_CFP100% 100ms, 1min int, 2.5hr)/';
savedir=[root 'Results' filesep];
load([savedir filesep 'Data_' num2str(option) '.mat'],'sensor','label','drugFrame');
%%
%1% 100ms, 1% 10ms, 1% 5ms
%for i=1:18
    %figure(i); imagesc(sensor(i).ERK,[0 1.5]); vline(drugFrame);
%end
%%
gate=0; normOption=1;
colorMap={'r','g','b','y','c','r','g','b','y','c','r','g','b','y','c','r','g','b','y','c','r','g','b','y','c','r','g','b','y','c'};
...
cells_name={'WM983B';};
ci = 0.95; alpha = 1 - ci;
numCol=round(length(sensor)/2);
figure; count=0;
for i=1:30 %length(sensor)
    gating=sensor(i).gating;
    
    if normOption
        plot_data=sensor(i).normERK;
    else
        plot_data=sensor(i).ERK;
    end
    
    if gate==1
        plot_data=plot_data(gating,:);
    end
    
    count=count+1;
    subplot(6,5,count); imagesc(plot_data,[yMin yMax]); colorbar; vline(drugFrame); colormap(parula_gray);
    set(gca,'Xtick',[-10:10:70],'XTickLabel',[-10:10:500],'fontsize',10,'tickdir','out','linewidth',2)
    ylabel('Cells','FontSize',10);xlabel('Time since inhibition (min)','FontSize',10);
    xlim([0 numFrame])
    
    title([label{i} ' n=' num2str(size(plot_data,1)) ' cells'],'fontsize',10);
end
figu(0.5,1);
%%
gate=0; normOption=1;
colorMap={'r','g','b','y','c','r','g','b','y','c','r','g','b','y','c','r','g','b','y','c','r','g','b','y','c','r','g','b','y','c','r','g','b','y','c','r','g','b','y','c','r','g','b','y','c','r','g','b','y','c','r','g','b','y','c','r','g','b','y','c'};

condition={'ctrl','MEK1','MEK2','MEK1/2','ERK1/2'};
...
for i=1:30
    %numCell=50;
    %numCell=size(sensor(i).ERK,1);

        
    if ismember(i,[1:5:60])
        counter=counter+1;
    end
    %data_mean=nanmean(sensor(i).ERK(1:numCell,:));
    gating=sensor(i).gating;
    ...
    data_mean=nanmean(plot_data);
    numCell(i)=size(plot_data,1);
   
    S=nanstd(plot_data);
    N=sum(~isnan(plot_data));%sum(obj.cellState);
    T_multiplier = tinv(1-alpha/2, N-1);
    ci95 = T_multiplier.*S./sqrt(N);
    
    subplot(2,3,counter);hold on;
    plot(data_mean,colorMap{i}); %plot(data_mean-ci95,colorMap{i}); plot(data_mean+ci95,colorMap{i});
    %shadedErrorBar(1:numFrame,data_mean,ci95,colorMap{i},1);
    vline(drugFrame);
    set(gca,'Xtick',[0:10:numFrame],'XTickLabel',[-10:10:300],'fontsize',12,'tickdir','out','linewidth',2);
    ylabel('ERK activity','FontSize',12);xlabel('Time (min)','FontSize',12);
%     if ismember(i,[4:4:18])
%         title(['Corrected ERK activity, Num Cell:' num2str(numCell(i-4)) 'cells ' num2str(numCell(i-3)) 'cells ' num2str(numCell(i-2)) 'cells ' num2str(numCell(i-1)) 'cells' num2str(numCell(i)) 'cells'],'fontsize',10);
%         ylim([yMin yMax]);
%         xlim([1 70]);
%         legend(label{i-4:i})
%     end
if ismember(i,[5:5:60])
       legend(condition)
    end
end
figu(0.5,1)
...
for i=1:8
    ...
    responder=sum(gating);
    total=length(gating);
    
    resRate(i)=responder/total;
end

figure;
bar(resRate)
figu(0.3,0.3)
set(gca,'Xtick',[0:25:numFrame],'XTickLabel',label,'fontsize',24,'tickdir','out','linewidth',2);

%%

subplot(1,3,2);hold on;
plot(nanmean(sensor(1).CDK4_corrected));
plot(nanmean(sensor(2).CDK4_corrected)); vline(drugFrame);
set(gca,'Xtick',[0:25:numFrame],'XTickLabel',[0:5:300],'fontsize',24,'tickdir','out','linewidth',2);
...
plot(nanmean(sensor(2).CDK2)); vline(drugFrame);
set(gca,'Xtick',[0:25:numFrame],'XTickLabel',[0:5:300],'fontsize',24,'tickdir','out','linewidth',2);
...
%%
yMax=[1.2]; yMax2=[0.85]; %fisrtFrame=5; drug_frame=105-fisrtFrame; lastFrame=105+100;
cells_name={'WM858';};
ci = 0.95; alpha = 1 - ci;
figure; count=0;
for i=1:length(sensor)
    count=count+1;
    subplot(2,2,count); imagesc(sensor(i).ERK_corrected,[0 yMax]); colorbar; vline(drugFrame); colormap(parula_gray);
    set(gca,'Xtick',[9:25:350],'XTickLabel',[-20:5:300],'fontsize',10,'tickdir','out','linewidth',2)
    ylabel('Cells','FontSize',10);xlabel('Time since inhibition (hr)','FontSize',10); title(label{i},'fontsize',20);
    xlim([9 209])
    
    ERK_median_1=nanmean(sensor(i).ERK_corrected);
    ...
    set(gca,'Xtick',[9:25:350],'XTickLabel',[-20:5:300],'fontsize',24,'tickdir','out','linewidth',2);
...
%%
yMax=[1.2]; yMax2=[0.8]; %fisrtFrame=5; drug_frame=105-fisrtFrame; lastFrame=105+100;
cells_name={'WM858';};
ci = 0.95; alpha = 1 - ci;
figure; count=0;
for i=1:length(sensor)
    count=count+1;
    subplot(2,2,count); imagesc(sensor(i).CDK4_corrected,[0 yMax]); colorbar; vline(drugFrame); colormap(parula_gray);
    set(gca,'Xtick',[9:25:350],'XTickLabel',[-20:5:300],'fontsize',10,'tickdir','out','linewidth',2)
    ylabel('Cells','FontSize',10);xlabel('Time since inhibition (hr)','FontSize',10); title(label{i},'fontsize',20);
    xlim([9 209])
    
    ERK_median_1=nanmean(sensor(i).CDK4_corrected);
    S=nanstd(sensor(i).ERK_corrected);
    N=sum(~isnan(sensor(i).ERK_corrected));%sum(obj.cellState);
    T_multiplier = tinv(1-alpha/2, N-1);
    ci95 = T_multiplier.*S./sqrt(N);
...
figu(1,0.5);
%%
yMax=[1.5 1.5]; yMax2=[1.4 1.4];
cells_name={'WM858'; 'WM938B';};

figure; count=0;
for i=1:length(sensor)
    
    ERK_plot=sensor(i).ERK_corrected(:,drugFrame-100:drugFrame+150);
    
    data=nanmean(ERK_plot);
...
for condi=1:3;
    ...
    D = sort_or_align_by_mitosis(data,divFrame,IF,IFdata,noMitosis)
    %% #1 Clear bad traces and short traces
    drug_frame=0;
...
%%
yMax=[1.5]; yMax2=[1.2]; %fisrtFrame=5; drug_frame=105-fisrtFrame; lastFrame=105+100;
cells_name={'WM858'; 'WM938B';};
ci = 0.95; alpha = 1 - ci;
figure; count=0;
for i=1:length(ERK_corrected)
    count=count+1;
    subplot(4,2,count); imagesc(ERK_corrected{i},[0 yMax]); colorbar; vline(drug_frame); colormap(parula_gray);
    ...
    ERK_median_1=nanmean(ERK_corrected{i});
...
yMax=[1.5 1.2]; yMax2=[3 1]; fisrtFrame=13; drug_frame=[145 413]-fisrtFrame;
cells_name={'WM858'; 'WM938B';};
ci = 0.95; alpha = 1 - ci;
figure; count=0;
for i=1:length(ERK_corrected)
    count=count+1;
    subplot(2,2,count); imagesc(ERK_corrected{i}(:,fisrtFrame:end),[0 yMax(i)]); colorbar; vline(drug_frame); colormap(parula_gray);
    set(gca,'Xtick',[0:100:numFrame],'XTickLabel',[0:20:300],'fontsize',24,'tickdir','out','linewidth',2)
    ylabel('Cells','FontSize',30);xlabel('Time (hr)','FontSize',30);title(['ERK activity n=' num2str(size(ERK_corrected{i},1))],'fontsize',30);
    
    ERK_median_1=nanmean(ERK_corrected{i}(:,fisrtFrame:end));
    ...
    ylabel('Cells','FontSize',30);xlabel('Time (hr)','FontSize',30);title(['ERK activity cell=' cells_name{i}],'fontsize',30);
end
figu(1,1);
...
for i=1:condnum
    count=count+1;
    subplot(1,condnum,count),hold on
...
figure;
for i=1:condnum;
    ERK_median_1=nanmean(sensor(i).ERK_corrected(sensor(i).CDK4high,:));
    ...
    plot(1:201,ERK_median_1,'g'); plot(1:201,ERK_median_1-ci95high,'g'); plot(1:201,ERK_median_1+ci95high,'g');
...
for i=1:condnum
    ...
    count=count+1;
    subplot(condnum,3,count); hold on;
...
for i=1:2
    ...
    subplot(1,condnum,i),hold on
    plot(ERK_median_1,'g'); plot(ERK_median_1-ci95,'g'); plot(ERK_median_1+ci95,'g');
